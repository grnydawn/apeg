...
from .expressions import (Literal, Regex, Sequence, OneOf, FirstmatchOf,
...
#from parsimonious.expressions import (Literal, Regex, Sequence, OneOf, FirstmatchOf,
#    Lookahead, Optional, ZeroOrMore, OneOrMore, Not, TokenMatcher,
#    expression)
...
class BootstrappingGrammar(Grammar):
    ...
    def _expressions_from_rules(self, rule_syntax, custom_rules):
        ...
        firstmatch_term = Sequence(Literal('/'), _, term, name='firstmatch_term')
...
class RuleVisitor(NodeVisitor):
    ...
    def visit_firstmatched(self, node, firstmatched):
        first_term, other_terms = firstmatched
        return FirstmatchOf(first_term, *other_terms)

    def visit_firstmatch_term(self, node, firstmatch_term):
        """Return just the term from an ``firstmatch_term``.

        We already know it's going to be firstmatched, from the containing ``firstmatched``.

        """
        slash, _, term = firstmatch_term
        return term
