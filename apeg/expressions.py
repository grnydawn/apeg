...
from .exceptions import ParseError, IncompleteParseError, AmbiguousGrammarError
...
class Expression(StrAndRepr):
    ...
    def _as_regex(self):
        """Return the regex representation of the rule.
...
class FirstmatchOf(Compound):
    ...
    def _uncached_match(self, text, pos, cache, error):
        for m in self.members:
            ...
            if node is not None:
                # Wrap the succeeding child in a node representing the FirstmatchOf:
...
class OneOf(Compound):
    ...
    Expressions are tested out of order. Only one should match. Otherwise fails.
